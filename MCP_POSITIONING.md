...
- **C1 — Unix primitives the agent already knows.** `ls`, `cd`, `cat`, `grep`, `find`, `tree`, `head`, `tail`, `wc` — zero new vocabulary. Every other doc-MCP invents its own query surface. **Status:** `evidenced` (2026-05-11). Full 4-tool surface + canonical Unix vocabulary exercised cleanly across **two hosts and two model classes**: Opus 4.7 on Claude Desktop (S1+S2+S3, `ls`/`tree`/`cat`/`head`/`sed`/`grep` plus `density` + proactive `stats`); Cursor Free-default (Sonnet 4.x-class) on Cursor (S1+S2+S3, same Unix surface plus stderr redirect `2>/dev/null` + shell `||` fallback). Zero docsvfs-specific syntax invented on either host. Honest caveat: both hosts use Anthropic-trained models; GPT or non-Anthropic cross-vendor still outstanding but not blocking. Evidence: [`evidence/layer-b/claude-desktop/`](evidence/layer-b/claude-desktop/) + [`evidence/layer-b/cursor/`](evidence/layer-b/cursor/).
- **C2 — Writable `/memory` + `/workspace` mounts with provenance.** `session_id` + `source` (`agent` / `tool` / `human` / `auto`) tagged on every write; janitor-safe; TTL on `/workspace`. No other docs MCP persists agent-authored notes across sessions. **Status:** `evidenced` (2026-05-11). Full chain (write → read → synthesize) proven on two hosts. Claude Desktop / Opus 4.7: write (S1, 3 calls, 10.2 KB), batched-cat read with content integration (S2), synthesis (S3, 13.1 KB runbook integrating 6 prior notes + 4 additive `/docs` reads). Cursor / Free-default: write (S1, 3 calls, 4.9 KB), list-and-acknowledge read (S2, awareness without content integration — prompt didn't force `cat`), full `cat`-and-synthesize (S3, 7 KB runbook + **first append-mode `remember` patch in any chain**, 6 of 6 prior notes read, line-range citations, 3 additive `/docs` reads). All three sub-claims evidenced on both hosts: write path (S1×2), read path (Claude Desktop S2 + Cursor S3), synthesis (S3×2). Honest caveat: Cursor S2 evidenced list-level but not content-level integration; S3 closed the gap. Evidence: [`evidence/layer-b/claude-desktop/S{1,2,3}.md`](evidence/layer-b/claude-desktop/) + [`evidence/layer-b/cursor/S{1,2,3}.md`](evidence/layer-b/cursor/).
...
- **2026-05-11 — uncommitted — Cursor S3 (onboarding runbook) — chain complete; C1 + C2 advance from `partially-evidenced` → `evidenced`**
  - **Why this is the headline move:** S3 was the diagnostic synthesis test. The S2 capture left an open question — *was the list-only behavior on Cursor S2 a model-class limitation or just because the S2 prompt didn't force `cat`?* S3 resolves it definitively. With the explicit `ls /memory` AND `cat` prior notes instruction, Cursor's Free-default model `cat`-read all 6 prior pins in full, cross-verified every claim against `/docs`, exercised the additive-memory pattern for the influence-function gap (Bergson, TracStar, ATTRIBUTION_RUNBOOK — same 3 files Opus pulled), and produced a synthesis with line-range citation precision *better than Opus's S3 runbook*. The integration gap from S2 was prompt-specific, not capability-specific. With all three sub-claims of C2 now evidenced on both hosts (write path × 2, read path on each, synthesis × 2), C2 meets the bar for `evidenced`. Same with C1 — the Unix vocabulary surface used identically across two model classes on two distinct hosts, three sessions each, zero docsvfs-specific syntax invented.
  - **What got captured:**
    - **Full `cat` of all 6 prior notes:** First `cat` was `/memory/dolma3-storage-layer-r2.md` — screenshot shows full content returned. Model statement: *"Reading all six `/memory` notes and verifying cited facts in `/docs`."* All 6 read in full.
    - **5 cross-verification reads against `/docs`:** `grep -n` for R2 bucket + endpoint (confirmed L24/L25), Modal volumes (L69–L72), 576-bin/blake2b/seed=42 (`WORKING_SAMPLE_DATA_ACCESS.md` L3–L14), storage-plane comparison (§4.1 L75–L89), sample staging (cross-reference DATA_INVENTORY + ATTRIBUTION_RUNBOOK).
    - **3 additive `/docs` reads for influence-function gap topic:** `BERGSON_REFERENCE.md` L1–L13, `TRACSTAR_PIPELINE.md` L1–L47, `ATTRIBUTION_RUNBOOK.md` L31–L170. The additive-memory pattern Opus demonstrated on his S3, duplicated on a weaker model.
    - **First append-mode `remember` call in any chain:** Model wrote the runbook as `remember` overwrite (6,835 B), then `cat`-read its own output back, found a vague citation precision issue, and called `remember` again with `append: true` to add a "Line fix" addendum (final size 6,997 B). The writable layer's append capability has been spec'd since v0.1; this is its first organic use by a frontier model. Useful product finding.
    - **S3 transcript:** [`evidence/layer-b/cursor/S3.md`](evidence/layer-b/cursor/S3.md). Final DB state expected: 7 rows / 13,895 B (Opus chain ended at 7 rows / 31,145 B — same row count, ~45% byte volume; consistent style across the Cursor chain).
  - **Three things Cursor did *better* than Opus on S3:** (1) append-mode patching for citation precision — Opus did one monolithic write; (2) line-range citations in the final runbook (L24–L25, L46–L71, etc.) instead of file-level; (3) explicit narration of the additive-memory pattern in the chat output.
  - **Three things Opus did better:** (1) ~0 pre-flight cost vs Cursor's ~13 built-in calls + reading DEMO_RUNS.md L1-120 to figure out the architecture; (2) proactive `stats` call (Cursor never used `stats` across the chain); (3) denser notes (13.1 KB single-shot vs ~7 KB).
  - **Changes to §2:**
    - **C1 → `evidenced` (2026-05-11):** Full 4-tool surface + canonical Unix vocabulary exercised cleanly across two hosts and two model classes. Honest caveat: both hosts use Anthropic-trained models; GPT or non-Anthropic cross-vendor still outstanding but not blocking.
    - **C2 → `evidenced` (2026-05-11):** All three sub-claims (write, read, synthesis) evidenced on both hosts. S2 on Cursor evidenced list-level integration only; S3 closed the gap by demonstrating full content-level integration.
    - C3 unchanged at `partially-evidenced` — threat model + adversarial-slug smoke still outstanding.
  - **Discoverability finding update across the full Cursor chain:** Built-in pre-flight calls went S1=~9, S2=~5, S3=~13. Non-monotonic — synthesis re-triggered full warmup. Schema-file reads (`remember.json`, `docs.json`) persisted every session. Host-shell `ls /memory` failures persisted (0 / 2 / 1). A `.cursor/rules` system-prompt snippet would shave all of these consistently; recommended addition to the project README + DocsVFS quickstart. **This is a recommended product follow-up, not a blocker.**
  - **Not yet shipped:** Threat-model write-up for C3, adversarial-slug smoke for C3, distribution Tier 1 (Smithery / `.mcpb`). All §4 distribution channels still `not-started`, but C1+C2 evidenced is what unlocks them. Cursor `after-S3.tsv` DB snapshot captured 2026-05-11 — **third byte-perfect match in the chain** (7 rows / 13,895 B, including the append-mode math: 6,835 overwrite + 162 append = 6,997 B for `data-attribution-onboarding.md`). End-to-end consistency between `remember` tool acks and SQLite persistence verified across all three Cursor sessions. Delta vs Ollama floor table in `DEMO_RUNS.md` now reflects all three chains (baseline / Claude Desktop / Cursor) side-by-side.
