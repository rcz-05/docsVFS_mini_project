...
- **2026-05-11 — uncommitted — Cursor S3 (onboarding runbook) — chain complete; C1 + C2 advance from `partially-evidenced` → `evidenced`**
  ...
  - **Not yet shipped:** Cursor `after-S3.tsv` DB snapshot (pre-computed in S3.md as 7 rows / 13,895 B; awaiting sqlite3 capture), Delta vs Ollama floor section update to include the full Cursor chain, threat-model write-up for C3, adversarial-slug smoke for C3, distribution Tier 1 (Smithery / `.mcpb`). All §4 distribution channels still `not-started`, but C1+C2 evidenced is what unlocks them.
